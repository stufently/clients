...
```typescript
// Single mapping
help: "question-circle", // help.svg → bwi-help

// Multiple mappings (one SVG, multiple class names)
collection: ["collection", "collection-shared"], // collection.svg → bwi-collection + bwi-collection
```
...
This generates both `.bwi-collection:before` and `.bwi-collection:before` using the same
