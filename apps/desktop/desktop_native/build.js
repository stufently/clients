...
function buildNapiModule(target, release = true, envExtra = {}) {
  ...
  const options = {
    stdio: "inherit",
    cwd: path.resolve(__dirname, "./napi"),
    shell: true,
  ...
  console.debug(
    "Running command:",
    "npm",
    ["run", "build", "--", crossCompileArg, releaseArg, targetArg].filter((s) => s != ""),
    options,
  );
...
if (!crossPlatform && !target) {
  ...
  const binPath = path.join(
    ...
    `desktop_proxy${process.platform === "win32" ? ".exe" : ""}`,
  );
...
if (target) {
  ...
  const ext = rustTargetsMap[target]?.platform === "win32" ? ".exe" : "";
  const binPath = path.join(
    __dirname,
    "target",
    target,
